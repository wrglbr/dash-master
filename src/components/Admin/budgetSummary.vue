  ...
  export default {
    ...
    methods: {
      depart_to(depart_id, slug){
        if(depart_id){
          this.$router.push(`/admin-department-budget-summary/${slug}/${depart_id}`);
        }
      },
      div_to(div_id, slug){
        if(div_id){
          this.$router.push(`/admin-division-budget-summary/${slug}/${div_id}`);
        }
      },
      formatPrice(value) {
      ...
      exporrt(pid){
        ...
        axios.get(`${config.apiUrl}/budget/exbs/${pid}/`, {
          ...
          link.href = url;
          link.setAttribute('download', `${this.object.period}.xls`);
